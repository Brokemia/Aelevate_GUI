...
namespace Aelevate {
    public class SpeedGraph : BindableObject, IDrawable {
        private static readonly NLog.Logger Logger = NLog.LogManager.GetCurrentClassLogger();
        private const int SPEED_COUNT = 20;
        private float maxSpeed = 0;
        private readonly List<float> pastSpeeds = new() { 0 };

        public void AddSpeed(float speed) {
            pastSpeeds.Insert(0, (float)speed);
        ...
        public SpeedGraph() {
            ArduinoComm.Instance.SpeedGraph = this;
        }
        ...
        public void Draw(ICanvas canvas, RectF dirtyRect) {
            ...
            canvas.DrawString(string.Format("{0:F2} mph", pastSpeeds[0]), dirtyRect.Left + dirtyRect.Width * 0.02f, dirtyRect.Top + dirtyRect.Height * 0.17f, HorizontalAlignment.Left);
            if (maxSpeed > 0) {
